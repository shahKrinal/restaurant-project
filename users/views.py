...
class CreateUser(APIView):
    ...
    def post(self, request):
        ...
        if is_permission(
            request.user,
            f"can_create_user_at_level{user_type}",
            restaurant,
            role=role,
    ...
    def get(self, request):
        ...
        queryset = self.queryset.filter(
            user_type__gt=request.user.user_type, restaurant__in=restaurants
        )
        if request.user.role.position == "Senior":
            queryset = queryset | User.objects.filter(
                Q(user_type=request.user.user_type, role__position="junior")
            )
...
class UserView(APIView):
    ...
    def get(self, request, pk):
        ...
        role = instance.role
        if is_permission(
            request.user, f"read_user_at_level{user_type}", res_id, role=role
        ):
    ...
    def patch(self, request, pk, *args, **kwargs):
        ...
        if is_permission(
            request.user,
            f"update_user_at_level{user_type}",
            restaurant,
            role,
            permissions,
        ):
            ...
            if user_type_from_request and user_permission not in str(
    ...
    def delete(self, request, pk, *args, **kwargs):
        ...
        role = instance.role
        if is_permission(
            request.user, f"delete_user_at_level{user_type}", res_id, role=role
        ):
